...
const Data = {
  ...
  async load() {
    ...
    try {
      ...
      console.log('真实 JSONBin 返回:', json);

      const record = json.record || {};
      if (typeof record !== 'object') {
        console.warn('record 无效，初始化为空');
        this.local = {};
      } else {
        this.local = record;
      }
...
const UI = {
  ...
  refresh() {
    ...
    if (!Data.local || typeof Data.local !== 'object' || Object.keys(Data.local).length === 0) {
      navTree.innerHTML = '<div style="padding:16px;color:#aaa;">暂无分类（可新增）</div>';
    } else {
...
const DOM = {
  ...
  addOption(parent, value, text) {
    ...
    o.value = value; o.textContent = text; parent.appendChild(o);
  },
